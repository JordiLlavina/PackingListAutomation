...
- **Mezclar en una caja exige el mismo cartón** (`AgrupadorCajas`): la caja *es* un cartón concreto, no una capacidad abstracta. Ninguna norma comercial cambia eso.
...
- **El reparto dentro de un artículo es equitativo**: 31 unidades con 10 por caja son 8+8+8+7, nunca 10+10+10+1 (una caja casi vacía se aplasta con el peso de las de encima).
- **Hijas y padres no van juntos**: el reparto y la prioridad se evalúan sobre la destinación **hija** (`CHINE FRANCH` se sirve de las primeras), pero cajas, palets y numeración van por destinación **padre** (`WHOLESALE`), que es quien recibe el bulto. La altura, en cambio, la hereda la hija de su padre.
- **La numeración es el último paso**, palet a palet y pila a pila, para que cada palet ocupe un rango contiguo por construcción. Sin eso `PaletAssignmentService` no podría reasignar los palets al importar el `EnvioInput` generado, y media docena de cajas se quedarían sin palet sin que nada avisara.
