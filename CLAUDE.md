...
- Los tests instancian los servicios con `new` (JUnit 5 puro); solo `PackingListApplicationTest` y `RutasTest` levantan contexto Spring (el segundo para comprobar el mapeo de rutas y las redirecciones). Los tests de builders reabren el `.xlsx` generado con POI y comprueban celdas reales — seguir ese patrón.
